...
def handle_exit(sig, frame):
    ...
    try:
        ...
        if cap and hasattr(cap, "isOpened") and cap.isOpened():
            cap.release()
            print("[INFO] Camera released.")
        elif PICAMERA_AVAILABLE: # If using Picamera2, it was never assigned to cap, so exit gracefully
             pass
        cv2.destroyAllWindows()
    except Exception as e:
        print(f"[ERROR] Cleanup failed: {e}")
...
# -----------------------------
# Frame processing (FIXED: Returns last_hour)
# -----------------------------
def process_frame(frame, last_hour):
    global video_writer
    now = datetime.now()
    
    # CRITICAL FIX 1: Hour change logic is now handled here and returned
    if now.hour != last_hour:
        if video_writer:
            video_writer.release()
        init_video_writer()
    ...
    if video_writer:
        # Ensure we write a correctly sized frame
        try:
            video_writer.write(cv2.resize(annotated, (frame_width, frame_height)))
        except cv2.error as e:
            print(f"[Error] Failed to write frame to video: {e}")
            
    # CRITICAL FIX 1: Return the updated last_hour
    return annotated, last_hour

# -----------------------------
# Camera capture (FIXED: Updates last_hour and uses .copy())
# -----------------------------
def camera_capture():
    global global_frame, video_writer, cap
    init_video_writer()
    last_hour = datetime.now().hour # Initial hour
    ...
    if PICAMERA_AVAILABLE:
        ...
        while True:
            # Capture frame and convert to BGR (OpenCV format)
            frame = picam2.capture_array()
            ...
            # Picamera2 often captures XBGR/BGRA, convert to standard BGR
            if frame.shape[2] == 4:
                 frame = cv2.cvtColor(frame, cv2.COLOR_BGRA2BGR)
            
            # CRITICAL FIX 1: Update last_hour from return value
            annotated, last_hour = process_frame(frame, last_hour)
            
            with frame_lock:
                # CRITICAL FIX 3: Use .copy() for thread safety
                global_frame = annotated.copy() 
            time.sleep(0.05)
    else:
        print("🎦 Using USB/OpenCV camera...")
        cap = cv2.VideoCapture(0)
        # Set dimensions for OpenCV camera
...
# -----------------------------
# Flask Streaming (FIXED: Added sleep on None frame)
# -----------------------------
@app.route("/video_feed")
def video_feed():
    def generate_frames():
        ...
        while True:
            with frame_lock:
                if global_frame is None:
                    # CRITICAL FIX 2: Sleep briefly if frame is not initialized
                    time.sleep(0.1) 
                    continue
                
                # Check for bad array type before imencode
            ...
            # Ensure encoding was successful before yielding
            if not ret:
                continue
                
            yield (b"--frame\r\nContent-Type: image/jpeg\r\n\r\n" + frame_bytes + b"\r\n")
            # Limit the frame rate for streaming
            time.sleep(0.05) 
...
if __name__ == "__main__":
    ...
    print(f"✅ Flask server running at: http://{ip}:5000")
    try:
        # Use socketio.run instead of app.run for compatibility
